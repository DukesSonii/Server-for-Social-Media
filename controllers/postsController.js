...
const addCommentController = async (req, res) => {
  try {
    ...
    if (!commentText) {
      return res.send(error(400, "Comment text is required"));
    }
    ...
    const updatedPost = await Post.findById(postId).populate({
      ...
      select: "name avatar", // Only populate 'name' and 'avatar'
    });

    return res.send(
      success(200, { post: mapPostOutput(updatedPost, req._id) })
    );
  } catch (e) {
    return res.send(error(500, e.message));
  }
...
const deleteCommentController = async (req, res) => {
  try {
    ...
    if (comment.author.toString() !== curUserId) {
      return res.send(error(403, "Only the author can delete their comment"));
    }
